    ...
    <main className="min-h-screen bg-[#0a0710]">
      ...
      <div className="pt-24 px-6 md:px-16 pb-12">
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 mb-8">
          {categories.map((cat) => (
            <button key={cat} className="flex-shrink-0 px-5 py-2 rounded-full bg-gray-800 text-white text-sm hover:bg-purple-500 hover:text-black transition font-medium">
              {cat}
            </button>
          ))}
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {landscapeVideos.map((video) => (
                <Link key={video.id} href={`/watch/${video.id}`} className="group">
                  <div className="aspect-video bg-gray-800 rounded-xl overflow-hidden group-hover:ring-2 ring-purple-500 transition-all duration-200">
                    {video.thumbnail_url
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
              {verticalVideos.map((video) => (
                <Link key={video.id} href={`/watch/${video.id}`} className="group">
                  <div className="aspect-[9/16] bg-gray-800 rounded-xl overflow-hidden group-hover:ring-2 ring-pink-500 transition-all duration-200">
                    {video.thumbnail_url
                      ? <img src={video.thumbnail_url} alt={video.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-200" />
                      : <div className="w-full h-full flex items-center justify-center"><span className="text-gray-600 text-xs">No thumbnail</span></div>
                    }
